// Reference: https://leetcode.com/problems/merge-triplets-to-form-target-triplet/
...
// - Choose the first and last triplets [[2,5,3],[1,8,4],[1,7,5]]. Update the last triplet to 
//      be [max(2,1), max(5,7), max(3,5)] = [2,7,5]. triplets = [[2,5,3],[1,8,4],[2,7,5]]
...
// Output: false
// Explanation: It is impossible to have [3,2,5] as an element because there is no 2 in any of 
//      the triplets.
...
// Output: true
// Explanation: Perform the following operations:
// - Choose the first and third triplets [[2,5,3],[2,3,4],[1,2,5],[5,2,3]]. Update the third 
//      triplet to be [max(2,1), max(5,2), max(3,5)] = [2,5,5]. triplets = [[2,5,3],[2,3,4],
//      [2,5,5],[5,2,3]].
// - Choose the third and fourth triplets [[2,5,3],[2,3,4],[2,5,5],[5,2,3]]. Update the fourth 
//      triplet to be [max(2,5), max(5,2), max(5,3)] = [5,5,5]. triplets = [[2,5,3],[2,3,4],
//      [2,5,5],[5,5,5]].
    ...
    bool mergeTriplets(vector<vector<int>>& triplets, vector<int>& target) {
        ...
        for (int i = 0; i < 3; ++i)
            for (vector<int> triplet: triplets)
                if ((target[i] == triplet[i]) && (triplet[(i+1)%3] <= target[(i+1)%3]) && (triplet[(i+2)%3] <= target[(i+2)%3]))
                    res[i] = triplet[i];
        return (res == target);
    }
};
