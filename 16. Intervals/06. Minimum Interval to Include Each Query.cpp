// Reference: https://leetcode.com/problems/minimum-interval-to-include-each-query/
    ...
    vector<int> minInterval(vector<vector<int>>& intervals, vector<int>& queries) {
        ...
        for (int qIdx = 0, intIdx = 0; qIdx < q.size(); ++qIdx) {
            while (intIdx < intervals.size() && intervals[intIdx][0] <= q[qIdx].first) {
                minHeap.push({intervals[intIdx][1] - intervals[intIdx][0] + 1, intervals[intIdx][1]});
                ++intIdx;
            }
            while (!minHeap.empty() && minHeap.top().second < q[qIdx].first)
                minHeap.pop();
            res[q[qIdx].second] = (minHeap.empty() ? -1 : minHeap.top().first);
        }
        return res;
    }
};
