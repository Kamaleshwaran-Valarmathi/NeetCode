...
//          endStation that happened directly, meaning a check in at startStation followed by a 
...
public:
    ...
    map<pair<string, string>, pair<double, int>> avgTimeMap; // {{startStation, endStation}, {totalTime, travelCount}}
    ...
    void checkOut(int id, string stationName, int t) {
        auto [checkInStation, checkInTime] = checkInMap[id];
        checkInMap.erase(id);
        avgTimeMap[{checkInStation, stationName}].first += (t - checkInTime);
        avgTimeMap[{checkInStation, stationName}].second++;
    }
    
    double getAverageTime(string startStation, string endStation) {
        auto [timeTaken, travelCount] = avgTimeMap[{startStation, endStation}];
        return (timeTaken / travelCount);
    }
};
