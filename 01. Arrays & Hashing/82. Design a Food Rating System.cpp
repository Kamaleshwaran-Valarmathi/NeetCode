...
class FoodRatings {
    unordered_map<string, map<int, set<string>, greater<int>>> cuisineRatedFoods;  // {cuisine, {rating, {food}}}
    unordered_map<string, string> foodCuisine;  // {food, cuisine}
    unordered_map<string, int> foodRatings;  // {food, rating}
...
public:
    FoodRatings(vector<string>& foods, vector<string>& cuisines, vector<int>& ratings) {
        for (int i = 0; i < foods.size(); ++i) {
            ...
            cuisineRatedFoods[cuisines[i]][ratings[i]].insert(foods[i]);
        }
    }
    
    void changeRating(string food, int newRating) {
        int prevRating = foodRatings[food];
        string cuisine = foodCuisine[food];

        foodRatings[food] = newRating;
        cuisineRatedFoods[cuisine][newRating].insert(food);

        cuisineRatedFoods[cuisine][prevRating].erase(food);
        if (cuisineRatedFoods[cuisine][prevRating].empty())
            cuisineRatedFoods[cuisine].erase(prevRating);
    }
    
    string highestRated(string cuisine) {
        return *(cuisineRatedFoods[cuisine].begin()->second.begin());
    }
};
