...
// number of points at position (r, c) on the matrix. Two robots are playing a game on this 
// matrix.
...
public:
    long gridGame(vector<vector<int>>& grid) {
        long topSum = accumulate(grid[0].begin(), grid[0].end(), 0l);
        long res = LONG_MAX, bottomSum = 0;
