...
public:
    ...
    void dfs(int x, int y, int row, int col, vector<vector<int>>& heights, vector<vector<bool>>& ocean) {
        for (int i = 0; i < X.size(); ++i) {
            int nextX = x + X[i], nextY = y + Y[i];
            if (isValid(nextX, nextY, row, col) && !ocean[nextX][nextY]) {
                if (heights[x][y] <= heights[nextX][nextY]) {
                    ocean[nextX][nextY] = true;
                    dfs(nextX, nextY, row, col, heights, ocean);
                }
            }
        }
    }
    ...
    vector<vector<int>> pacificAtlantic(vector<vector<int>>& heights) {
        ...
        for (int i = 0; i < row; ++i)
            pacific[i][0] = atlantic[i][col-1] = true;
        for (int i = 0; i < col; ++i)
            pacific[0][i] = atlantic[row-1][i] = true;
