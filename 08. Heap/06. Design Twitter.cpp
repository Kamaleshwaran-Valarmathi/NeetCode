...
public:
    ...
    vector<pair<int, int>> feed; // {usedId, tweetId}
    ...
    vector<int> getNewsFeed(int userId) {
        vector<int> curFeed;
        for (int i = feed.size()-1; i >= 0 && curFeed.size() < 10; --i)
            if (feed[i].first == userId || followers[userId].find(feed[i].first) != followers[userId].end())
                curFeed.push_back(feed[i].second);
        return curFeed;
    }
