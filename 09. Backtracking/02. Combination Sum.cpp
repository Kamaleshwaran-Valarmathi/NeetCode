...
class Solution {
public:
    void helper(int idx, vector<int>& nums, int target, int curSum, vector<int>& cur, vector<vector<int>>& res) {
        ...
        helper(idx, nums, target, curSum + nums[idx], cur, res);
        cur.pop_back();
        helper(idx+1, nums, target, curSum, cur, res);
    }
