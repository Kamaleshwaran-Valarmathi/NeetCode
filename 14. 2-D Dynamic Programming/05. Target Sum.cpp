...
class Solution {
public:
    int helper(int idx, int sum, vector<int>& nums, int target, map<pair<int, int>, int>& dp) {
        if (idx == nums.size())
            return (sum == target) ? 1 : 0;
        if (dp.find({idx, sum}) != dp.end())
            return dp[{idx, sum}];
        int positive = helper(idx+1, sum+nums[idx], nums, target, dp);
        int negative = helper(idx+1, sum-nums[idx], nums, target, dp);
