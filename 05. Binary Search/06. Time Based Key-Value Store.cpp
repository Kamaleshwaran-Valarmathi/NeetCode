...
public:
    ...
    string get(string key, int timestamp) {
        if (timeMap.find(key) == timeMap.end() || timeMap[key][0].second > timestamp)
            return "";
        int prev = 0, left = 0, right = timeMap[key].size()-1;
        vector<pair<string, int>>& value = timeMap[key];
        while (left <= right) {
            int mid = (left + right) >> 1;
            if (value[mid].second == timestamp)
                return value[mid].first;
            (value[mid].second < timestamp) ? (left = mid+1, prev = mid) : (right = mid-1);
        }
        return value[prev].first;
    }
};
