...
class Solution {
public:
    void dfs(TreeNode* root, int curMax, int& goodNodesCount) {
        if (root == NULL)
            return;
        if (curMax <= root->val)
            ++goodNodesCount;
        dfs(root->left, max(curMax, root->val), goodNodesCount);
        dfs(root->right, max(curMax, root->val), goodNodesCount);
    }

    int goodNodes(TreeNode* root) {
        int goodNodesCount = 0;
        dfs(root, INT_MIN, goodNodesCount);
